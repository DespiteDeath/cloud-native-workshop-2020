...
sidecar-контейнер, который перехватывает, проверяет или функционально усиливает соединение.!!!

Istio добавляет route-ы по умолчанию между Kubernetes сервисами.
Тем не менее рекомендуется явно добавлять route-ы Istio между желаемыми сервисами.
